...
export const logSerializer = (
  ...
  meta?: ILogSerializerMeta,
  statusCode?: number
): Object => {
  const method = req.method;
  const url = req.originalUrl;
  const status = statusCode ? statusCode : res.statusCode;
  const responseTimeFromHeader = res.getHeader("X-Response-Time")
    ? res.getHeader("X-Response-Time")
    : "0.00ms";
  const responseTime = responseTimeFromHeader
    ? parseFloat(responseTimeFromHeader.toString())
    : undefined;
