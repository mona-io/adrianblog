...
export async function authSignInService(params: IAuthenticateService) {
  const { userEmail, userPassword, userIpAddress, userAgent } = params;

  const user = await User.findOne({ email: userEmail });
  if (!user) {
    throw new BadRequestError(
      "Invalid Crendentials",
      AuthLocaleEnum.ERROR_BADREQ_PASS_MISMATCH
    );
  }
  ...
  if (isSuspended) {
    throw new BadRequestError(
      "User is suspended",
      AuthLocaleEnum.ERROR_USER_IS_SUSPENDED
    );
  }
