...
export async function taxonomyEditService(data: ITaxonomyEditService) {
  ...
  if (parent && !parentDoc) {
    throw new BadRequestError(
      "Parent taxonomy not found",
      TaxonomyLocaleEnum.ERROR_PARENT_NOT_FOUND
    );
  }

  const duplicateTaxonomy = await Taxonomy.findOne({
    type,
    term,
    id: { $ne: taxonomy.id },
  });
  if (duplicateTaxonomy) {
    throw new BadRequestError(
      "Duplicated taxonomy is not allowed",
  ...
  if (parentDoc && !parentDoc.children.includes(taxonomy.id)) {
    parentDoc.set({
      children: parentDoc.children
        ? [...parentDoc.children, taxonomy.id]
        : [taxonomy.id],
    });
